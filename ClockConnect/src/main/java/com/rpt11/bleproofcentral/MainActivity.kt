...
class MainActivity : AppCompatActivity() {
    ...
    private val ssidTextView: TextInputEditText
        get() = findViewById(R.id.ssid)
    private val passwordTextView: TextInputEditText
        get() = findViewById(R.id.password)
    private val alarmTextView: TextInputEditText
        get() = findViewById(R.id.alarm)
    ...
    fun onTapWriteSsidAndPassword(view: View) {
        var gatt = connectedGatt ?: run {
            appendLog("ERROR: write failed, no connected device")
            return
        }
        ...
        characteristic.writeType = BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT
        characteristic.value = getString(R.string.ssid_password_prefix, ssidTextView.text.toString().trim(),passwordTextView.text.toString().trim()).toByteArray(Charsets.UTF_8)
        gatt.writeCharacteristic(characteristic)
    }

    fun onTapWriteAlarm(view: View) {
        var gatt = connectedGatt ?: run {
            appendLog("ERROR: write failed, no connected device")
            return
        }
        ...
        characteristic.writeType = BluetoothGattCharacteristic.WRITE_TYPE_DEFAULT
        characteristic.value = getString(R.string.alarm_prefix, alarmTextView.text.toString().trim()).toByteArray(Charsets.UTF_8)
        gatt.writeCharacteristic(characteristic)
    }
